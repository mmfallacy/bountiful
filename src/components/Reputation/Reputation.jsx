...
export default function Reputation({ score }) {
  const medals = [];
  for (let i = 0; i < Math.floor(score); i++) medals.push(<FullMedalSvg />);
  if (score % 1 === 0.5) medals.push(<HalfMedalSvg />);
  for (let i = 0; i < Math.floor(5 - score); i++) medals.push(<NewMedalSvg />);
